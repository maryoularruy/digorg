...
class DKPhotoImagePreviewVC: DKPhotoBaseImagePreviewVC {
    ...
    @objc private func downloadOriginalImage() {
        if let extraInfo = self.item.extraInfo, let originalURL = extraInfo[MediaCarouselItemExtraInfoKeyRemoteImageOriginalURL] as? URL {
    ...
    private func updateDownloadOriginalButtonTitle() {
        if let extraInfo = self.item.extraInfo, let fileSize = extraInfo[MediaCarouselItemExtraInfoKeyRemoteImageOriginalSize] as? UInt {
            self.downloadOriginalImageButton.setTitle(MediaCarouselResource.localizedStringWithKey("preview.image.download.original.title") + "(\(self.formattedFileSize(fileSize)))", for: .normal)
        } else {
            self.downloadOriginalImageButton.setTitle(MediaCarouselResource.localizedStringWithKey("preview.image.download.original.title"),
                                                      for: .normal)
        }
