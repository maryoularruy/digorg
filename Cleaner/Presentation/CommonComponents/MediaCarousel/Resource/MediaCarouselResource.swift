...
/// Manage all resource files and internationalization support for MediaCarousel.
public class MediaCarouselResource {
    ...
    public class func localizedStringWithKey(_ key: String, value: String? = nil) -> String {
        ...
        return string ?? NSLocalizedString(key, tableName: "MediaCarousel",
    ...
    private class func imageForResource(_ name: String) -> UIImage {
       // let bundle = Bundle.photoGalleryResourceBundle()
        let image = UIImage(named: name, in: nil, compatibleWith: nil) ?? UIImage()
...
private extension Bundle {
    class func photoGalleryResourceBundle() -> Bundle {
        let assetPath = Bundle(for: MediaCarouselResource.self).resourcePath!
        return Bundle(path: (assetPath as NSString).appendingPathComponent("MediaCarousel.bundle"))!
    }
}
