...
public class AppLocker: UIViewController {
    ...
    fileprivate var mode: ALMode = .validate {
        didSet {
            switch mode {
            case .create:
                messageLabel.text = "Create a new password" // Your submessage for create mode
            case .change:
                messageLabel.text = "Enter your password" // Your submessage for change mode
            case .deactive:
                messageLabel.text = "Enter your password" // Your submessage for deactive mode
            case .validate:
                messageLabel.text = "Confirm your password" // Your submessage for validate mode
    ...
    private func drawing(isNeedClear: Bool, tag: Int? = nil) { // Fill or cancel fill for indicators
        ...
        UIView.animate(withDuration: ALConstants.duration, animations: {
            pinView?.backgroundColor = isNeedClear ? Asset.bgOnCards.color : Asset.blue.color
        }) { _ in
    ...
    private func createModeAction() {
        if isFirstCreationStep {
            ...
            reservedPin = pin
            clearView()
            messageLabel.text = "Confirm your password"
        } else {
            confirmPin()
        }
    }
    ...
    private func incorrectPinAnimation() {
        pinIndicators.forEach { view in
            ...
            view.backgroundColor = Asset.bgOnCards.color
        }
